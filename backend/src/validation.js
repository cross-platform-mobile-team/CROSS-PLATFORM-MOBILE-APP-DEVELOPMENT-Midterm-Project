...
export function task(value, now) {
  ...
  if (dueDate !== null && !validCalendarDate(dueDate)) {
  ...
  if (typeof createdAt !== 'string' || !/^\d{4}-\d{2}-\d{2}T.*(Z|[+-]\d{2}:\d{2})$/.test(createdAt) ||
      !validCalendarDate(createdAt.slice(0, 10)) || !Number.isFinite(Date.parse(createdAt))) {
